...
router.get("/:comment_id/edit", middleware.checkCommentOwnership, (req,res)=> {
	...
	Recordshop.findById(req.params.id, (err,foundRecordshop)=>{
		...
		if(err) {
			...
			res.render("comments/edit",{recordshop_id:req.params.id,comment:foundComment})
		}
	})
		}
	})	
...
router.put("/:comment_id",middleware.checkCommentOwnership, (req,res)=>{
	...
	Recordshop.findById(req.params.id, (err,foundRecordshop)=> {      // NEEDED for manual url manipulation
		if(err || !foundRecordshop) {
			req.flash("error","Can't find recordshop")
			res.redirect("back")
		} else {

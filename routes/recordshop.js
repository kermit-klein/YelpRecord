  ...
  if (req.query.search) {
    ...
    Recordshop.find({ name: regex }, function (err, allRecordshops) {
      if (err) {
        ...
        res.render("recordshops/index", {
          recordshops: allRecordshops,
          page: "recordshops",
          noMatch: noMatch,
        });
      }
...
router.get("/:id", (req, res) => {
  ...
  Recordshop.findById(req.params.id)
...
router.put(
  "/:id",
  middleware.checkRecordshopOwnership,
  upload.single("image"),
  (req, res) => {
...
router.delete("/:id", middleware.checkRecordshopOwnership, (req, res) => {
  Recordshop.findByIdAndRemove(
    req.params.id,
    async (err, deletedrecordshop) => {
      if (err) {
        req.flash("error", err.message);
